# 12005 — Secrets, Credentials & Connector Security V1

## Purpose

Protect API keys, OAuth tokens, service credentials, certificates, and connector sessions.
...
- store in managed secret vault
- no plaintext in source control
- no plaintext in prompts
...
- Secrets never appear in ordinary logs.
- Connector scopes are visible.
- Revoked credentials stop access.
- Rotation does not silently break production.
...
Canonical operations: `POST /v1/security/12005/records`, `GET /v1/security/12005/records/{id}`, `POST /v1/security/12005/records/{id}/submit`, `POST /v1/security/12005/records/{id}/approve`, `POST /v1/security/12005/records/{id}/suspend`, and `POST /v1/security/12005/records/{id}/retire`.

Required events: `12005.record.created`, `12005.record.submitted`, `12005.record.approved`, `12005.control.failed`, `12005.record.suspended`, `12005.exception.opened`, and `12005.record.retired`. Events include event ID, tenant, actor, object version, correlation ID, timestamp, classification, and evidence references.
