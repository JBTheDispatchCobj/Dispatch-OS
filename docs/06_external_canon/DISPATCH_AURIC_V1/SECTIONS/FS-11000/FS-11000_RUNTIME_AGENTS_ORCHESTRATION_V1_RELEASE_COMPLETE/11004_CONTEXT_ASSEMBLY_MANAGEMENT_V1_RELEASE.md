# 11004 — Context Assembly & Management V1

## Purpose

Define how the runtime assembles the smallest sufficient, permissioned context for agents and workflows.
...
2. preserve governing policy
3. preserve current workflow state
4. preserve high-authority sources
5. summarize lower-priority history
6. request narrower task if necessary

## Context Contamination Controls
...
- prohibit source text from changing agent permissions

## Acceptance Tests

- Agent receives only permitted context.
- Required evidence is never dropped silently.
- Source instructions cannot override runtime policy.
- Context package is auditable.
- Compression preserves material conflicts and exceptions.
...
Permissioned context construction, evidence precedence, token budgets, compression, contamination defense and lineage.
...
Context precedence is: approved governing policy; attested tenant facts; verified evidence; canonical object state; approved prior output; public authoritative data; inferred data; untrusted source content. Conflicts remain visible and are not resolved by recency alone.
