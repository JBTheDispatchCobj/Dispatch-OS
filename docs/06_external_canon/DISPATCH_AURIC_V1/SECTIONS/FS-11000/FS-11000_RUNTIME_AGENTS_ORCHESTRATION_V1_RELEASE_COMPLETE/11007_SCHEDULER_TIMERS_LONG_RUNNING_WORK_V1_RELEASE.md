# 11007 — Scheduler, Timers & Long-Running Work V1

## Purpose

Define scheduled jobs, timers, recurring workflows, deadlines, waits, and resumable long-running execution.
...
- display user/institution timezone
- daylight-saving aware
- exact deadlines use explicit timezone

## Acceptance Tests

- Scheduled work survives restart.
- Misfire behavior is configurable.
- Duplicate recurring runs are controlled.
- Long waits preserve workflow state.
- Expired approvals and evidence trigger events.
...
Schedules, timers, leases, checkpoints, resumability, missed-run behavior and human wait states.
...
Scheduled jobs use tenant timezone plus stored UTC next-run time. Daylight-saving changes, missed runs, overlapping runs, lease expiry, cancellation, and backfill are explicit policy decisions.
