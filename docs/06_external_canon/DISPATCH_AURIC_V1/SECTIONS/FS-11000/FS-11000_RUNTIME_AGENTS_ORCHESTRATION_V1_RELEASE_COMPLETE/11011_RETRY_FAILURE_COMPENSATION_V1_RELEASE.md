# 11011 — Retry, Failure & Compensation V1

## Purpose

Define predictable handling of transient errors, permanent failures, partial completion, and reversible actions.
...
- block inappropriate completion
- allow resume
- record exception
...
- retract draft notification
- reverse temporary status
- cancel queued job
- invalidate unapproved report
- restore prior object version

Financial/legal actions may require domain-specific reversal and cannot rely on generic compensation.
...
Error taxonomy, retry safety, circuit breakers, partial completion, compensation and replay.
...
Retries are permitted only when operation semantics and idempotency are known. Compensation is a new governed action, not deletion of history, and may itself require approval.
