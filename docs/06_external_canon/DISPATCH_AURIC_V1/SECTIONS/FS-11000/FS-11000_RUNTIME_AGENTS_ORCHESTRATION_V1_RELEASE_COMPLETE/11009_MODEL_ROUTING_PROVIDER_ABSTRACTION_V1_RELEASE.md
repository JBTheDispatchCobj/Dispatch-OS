# 11009 — Model Routing & Provider Abstraction V1

## Purpose

Define how Dispatch selects language, reasoning, embedding, extraction, classification, and local models without coupling workflows to one provider.
...
- restricted data uses approved private/local route
- regulated conclusions require human review regardless of model
- high-risk tasks prefer evaluated models
- low-cost models may handle simple extraction
- provider outage allows approved fallback
- model changes are versioned and tested
...
- Workflow definitions remain provider agnostic.
- Data classification constrains routing.
- Fallbacks are approved and auditable.
- Model changes do not silently alter production behavior.
- Cost and quality metrics are captured.
...
Capability-based routing, policy constraints, provider fallback, cost/quality tradeoffs and evaluation gates.
...
Routing first applies policy eligibility, then capability and quality thresholds, then latency and cost optimization. Fallback never sends a higher data classification to a provider not approved for that classification.
