# 13011 — Marketing, Publication & Demand Engine V1

## Purpose

Define how The Auric publication, institutional intelligence, market maps, events, and education create qualified network participation and demand.
...
- Market
- Network
- Innovation / “Two Dollar Bill”
- institutional profiles
- vendor/fintech intelligence
- regulatory change
- participation and capital opportunities
- operating playbooks
- executive education
...
→ meeting  
→ pilot  
→ Terminal/workflow adoption

## Editorial Controls

- source citations
- fact checking
- no confidential tenant data
- no unsupported investment claims
- no misleading AUM language
- legal/compliance review where required
- clear sponsored/partner disclosure
- version/archive
...
- Published facts are sourced.
- Private data is excluded.
- CTAs map to real offers.
- Attribution distinguishes correlation from direct conversion.
- Editorial review is auditable.
---
...
Canonical states: **idea, assigned, draft, fact_check, review, approved, scheduled, published, corrected, archived**.

1. Creation begins in `idea` and confers no execution or commercial authority.
...
Minimum evidence includes: source citations, fact-check record, editorial approval, disclosure review, channel proof, performance report. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13011/content_items`
- `GET /v1/commercial/13011/content_items/{id}`
- `PATCH /v1/commercial/13011/content_items/{id}`
- `POST /v1/commercial/13011/content_items/{id}/submit`
- `POST /v1/commercial/13011/content_items/{id}/approve`
- `POST /v1/commercial/13011/content_items/{id}/suspend`
- `POST /v1/commercial/13011/content_items/{id}/retire`
...
Required events: `content.created`, `content.review_requested`, `content.approved`, `content.published`, and `content.archived`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
