# 13003 — Pilot Qualification & Scoping V1

## Purpose

Define which organizations and use cases are appropriate for a Dispatch pilot and how pilot scope is bounded.
...
- executive sponsor
- specific institutional pain
- workflow owner
- available source data
- measurable baseline
- acceptable security path
- human approval path
- realistic users
- controlled output
...
- no sponsor
- undefined problem
- expectation of autonomous regulated approval
- unavailable or prohibited data
- no security owner
- no measurable outcome
- uncontrolled production write requirement
- incompatible legal or procurement constraints
...
- no final credit/investment/compliance/legal decision
- no external send without approval
- no production write unless approved
- no unapproved model/provider
- no out-of-scope data
- no hidden expansion of users or objects
...
- Scope identifies exclusions.
- Baseline and success metrics exist.
- Human authority remains intact.
- Pilot can be terminated cleanly.
---
...
Canonical states: **intake, qualified, scoping, review, approved, declined, active, completed, terminated**.
...
Minimum evidence includes: executive sponsor attestation, problem baseline, data inventory, security feasibility, success metric definition, pilot charter. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13003/pilot_scopes`
- `GET /v1/commercial/13003/pilot_scopes/{id}`
- `PATCH /v1/commercial/13003/pilot_scopes/{id}`
- `POST /v1/commercial/13003/pilot_scopes/{id}/submit`
- `POST /v1/commercial/13003/pilot_scopes/{id}/approve`
- `POST /v1/commercial/13003/pilot_scopes/{id}/suspend`
- `POST /v1/commercial/13003/pilot_scopes/{id}/retire`
...
Required events: `pilot.intake_created`, `pilot.qualified`, `pilot.scope_submitted`, `pilot.scope_approved`, and `pilot.terminated`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
