# 13008 — Partner, Channel & Ecosystem V1

## Purpose

Define partnerships that extend distribution, credibility, data, implementation, and transaction capability.
...
- law firm
- audit/compliance firm
- data provider
- capital partner
- sponsor bank
- wealth/insurance partner
- integration partner
...
- Partner scope and economics are explicit.
- Conflicts are identified.
- Shared data and relationships are permissioned.
- Partner performance is measured.
...
This volume is the binding V1 specification for **Partner, Channel & Ecosystem V1**. It governs internal Auric operators, customers, implementation partners, automated services, and agents. Local statements of work and cartridge configurations may narrow scope or strengthen controls but may not bypass tenant isolation, evidence requirements, human decision authority, pricing approval, security review, or immutable audit history.

## Canonical Object and Field Dictionary

The primary object is `partner`. Every record MUST include `id`, `tenant_id`, `object_type`, `version`, `status`, `classification`, `owner_id`, `created_at`, `updated_at`, `effective_at`, `expires_at`, `source_refs`, `evidence_refs`, `approval_refs`, `control_refs`, `audit_ref`, `correlation_id`, and `idempotency_key`. Required domain fields are defined in `partner.schema.json`. Monetary fields MUST identify currency and basis; percentages MUST identify denominator and period; dates MUST use ISO 8601; unknown, stale, inferred, attested, synthetic, and verified values MUST remain distinguishable.
...
Canonical states: **prospect, qualified, due_diligence, contracted, enabled, active, suspended, terminated**.

1. Creation begins in `prospect` and confers no execution or commercial authority.
...
Minimum evidence includes: partner profile, due diligence, conflict check, agreement, certification, tenant authorization. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13008/partners`
- `GET /v1/commercial/13008/partners/{id}`
- `PATCH /v1/commercial/13008/partners/{id}`
- `POST /v1/commercial/13008/partners/{id}/submit`
- `POST /v1/commercial/13008/partners/{id}/approve`
- `POST /v1/commercial/13008/partners/{id}/suspend`
- `POST /v1/commercial/13008/partners/{id}/retire`
...
Required events: `partner.qualified`, `partner.contracted`, `partner.enabled`, `partner.suspended`, and `partner.terminated`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
