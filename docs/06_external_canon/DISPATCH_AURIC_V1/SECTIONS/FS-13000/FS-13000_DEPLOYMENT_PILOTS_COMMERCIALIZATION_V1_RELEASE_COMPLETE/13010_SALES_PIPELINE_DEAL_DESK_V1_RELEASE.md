# 13010 — Sales Pipeline & Deal Desk V1

## Purpose

Define commercial opportunity management from target to contract and expansion.
...
- Sales stage reflects actual evidence.
- Custom scope receives delivery review.
- Pricing receives margin review.
- Security/procurement blockers are visible.
- Closed-won work creates implementation workflow.
---
...
Canonical states: **target, engaged, qualified, discovery, solution, security_procurement, proposal, negotiation, committed, closed_won, closed_lost, renewal**.

1. Creation begins in `target` and confers no execution or commercial authority.
...
Minimum evidence includes: qualification evidence, discovery notes, solution scope, capacity review, pricing approval, next action. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13010/commercial_opportunitys`
- `GET /v1/commercial/13010/commercial_opportunitys/{id}`
- `PATCH /v1/commercial/13010/commercial_opportunitys/{id}`
- `POST /v1/commercial/13010/commercial_opportunitys/{id}/submit`
- `POST /v1/commercial/13010/commercial_opportunitys/{id}/approve`
- `POST /v1/commercial/13010/commercial_opportunitys/{id}/suspend`
- `POST /v1/commercial/13010/commercial_opportunitys/{id}/retire`
...
Required events: `opportunity.created`, `opportunity.stage_changed`, `deal_desk.requested`, `proposal.issued`, and `opportunity.closed_lost`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
