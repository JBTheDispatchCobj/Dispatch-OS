# 13004 — Pilot Implementation Playbook V1

## Purpose

Define the repeatable implementation process for a controlled institutional pilot.
...
- compare baseline/outcome
- review adoption
- review quality
- review risk
- calculate value
- decide scale, extend, or stop
...
Week 2:
- profile, data, workflow, permissions

Week 3:
- testing, training, dashboards

Week 4:
- controlled launch and value review

## Acceptance Tests

- Implementation has named owners.
- Security and data work precede launch.
- UAT tests approvals and evidence.
- Pilot metrics are measured.
...
Canonical states: **planned, discovery, governance, configuration, validation, launch_ready, live, evaluation, closed**.

1. Creation begins in `planned` and confers no execution or commercial authority.
...
Minimum evidence includes: approved charter, RACI, data map, test results, training record, launch approval, value report. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13004/pilot_implementations`
- `GET /v1/commercial/13004/pilot_implementations/{id}`
- `PATCH /v1/commercial/13004/pilot_implementations/{id}`
- `POST /v1/commercial/13004/pilot_implementations/{id}/submit`
- `POST /v1/commercial/13004/pilot_implementations/{id}/approve`
- `POST /v1/commercial/13004/pilot_implementations/{id}/suspend`
- `POST /v1/commercial/13004/pilot_implementations/{id}/retire`
...
Required events: `implementation.started`, `implementation.phase_completed`, `uat.failed`, `launch.approved`, and `pilot.evaluated`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
