# 13009 — Go-to-Market & Institution Segmentation V1

## Purpose

Define target segments, entry points, messaging, outreach, qualification, and pilot conversion.

## Initial Segments

### Credit Unions

- small institutions needing managed operating support
- mid-sized institutions with fragmented systems
- large institutions seeking network/innovation leverage
- corporate credit unions
- CUSOs
- institutions with commercial/treasury ambitions
- institutions approaching exams or vendor renewals

### Fintechs/Vendors

- seeking credit-union pilots
- needing readiness/integration support
- seeking institutional profiles
- seeking compliant distribution
- seeking strategic capital/partnerships

### Capital Participants

- participation buyers/sellers
- CUSO investors
- venture/private-credit partners
- strategic acquirers
...
- institutional context in one place
- governed AI-enabled workflows
- opportunity detection
- evidence and approval
- managed configuration
- network participation
- no requirement to build an AI stack internally

## Acceptance Tests

- Segments map to specific entry offers.
- Outreach uses evidence-backed institutional context.
- Qualification identifies sponsor and pain.
- Messaging does not overclaim deployment or AUM.
- Pilot conversion path is explicit.
---
...
This volume is the binding V1 specification for **Go-to-Market & Institution Segmentation V1**. It governs internal Auric operators, customers, implementation partners, automated services, and agents. Local statements of work and cartridge configurations may narrow scope or strengthen controls but may not bypass tenant isolation, evidence requirements, human decision authority, pricing approval, security review, or immutable audit history.

## Canonical Object and Field Dictionary

The primary object is `market_segment`. Every record MUST include `id`, `tenant_id`, `object_type`, `version`, `status`, `classification`, `owner_id`, `created_at`, `updated_at`, `effective_at`, `expires_at`, `source_refs`, `evidence_refs`, `approval_refs`, `control_refs`, `audit_ref`, `correlation_id`, and `idempotency_key`. Required domain fields are defined in `market_segment.schema.json`. Monetary fields MUST identify currency and basis; percentages MUST identify denominator and period; dates MUST use ISO 8601; unknown, stale, inferred, attested, synthetic, and verified values MUST remain distinguishable.
...
Canonical states: **hypothesis, researched, approved, active, measuring, paused, retired**.

1. Creation begins in `hypothesis` and confers no execution or commercial authority.
...
Minimum evidence includes: ICP evidence, market sizing, message map, offer map, campaign plan, performance review. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13009/market_segments`
- `GET /v1/commercial/13009/market_segments/{id}`
- `PATCH /v1/commercial/13009/market_segments/{id}`
- `POST /v1/commercial/13009/market_segments/{id}/submit`
- `POST /v1/commercial/13009/market_segments/{id}/approve`
- `POST /v1/commercial/13009/market_segments/{id}/suspend`
- `POST /v1/commercial/13009/market_segments/{id}/retire`
...
Required events: `segment.created`, `segment.approved`, `campaign.launched`, `segment.performance_reviewed`, and `segment.retired`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
