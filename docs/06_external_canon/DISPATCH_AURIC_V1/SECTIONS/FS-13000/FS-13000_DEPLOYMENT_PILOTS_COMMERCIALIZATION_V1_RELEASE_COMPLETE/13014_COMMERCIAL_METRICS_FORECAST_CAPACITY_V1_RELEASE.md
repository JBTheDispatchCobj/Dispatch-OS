# 13014 — Commercial Metrics, Forecast & Capacity V1

## Purpose

Define metrics for pipeline, delivery, revenue, margin, retention, network growth, and implementation capacity.
...
- usage revenue
- data/pass-through revenue
- transaction revenue where permitted
- partner revenue
- expansion revenue
...
- available implementation staff
- partner capacity
- connector complexity
- workflow complexity
- security/procurement load
- support load
- model/hosting capacity

## Acceptance Tests

- Revenue categories are distinct.
- Pipeline forecast reflects stage evidence.
- Delivery capacity constrains sales commitments.
- Variable AI and data costs are captured.
- Retention metrics link to actual usage and value.
---
...
The primary object is `commercial_metric_snapshot`. Every record MUST include `id`, `tenant_id`, `object_type`, `version`, `status`, `classification`, `owner_id`, `created_at`, `updated_at`, `effective_at`, `expires_at`, `source_refs`, `evidence_refs`, `approval_refs`, `control_refs`, `audit_ref`, `correlation_id`, and `idempotency_key`. Required domain fields are defined in `commercial_metric_snapshot.schema.json`. Monetary fields MUST identify currency and basis; percentages MUST identify denominator and period; dates MUST use ISO 8601; unknown, stale, inferred, attested, synthetic, and verified values MUST remain distinguishable.
...
Canonical states: **draft, calculated, reconciled, approved, published, superseded**.
...
Minimum evidence includes: source extracts, calculation trace, reconciliation, forecast assumptions, capacity signoff, management approval. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13014/commercial_metric_snapshots`
- `GET /v1/commercial/13014/commercial_metric_snapshots/{id}`
- `PATCH /v1/commercial/13014/commercial_metric_snapshots/{id}`
- `POST /v1/commercial/13014/commercial_metric_snapshots/{id}/submit`
- `POST /v1/commercial/13014/commercial_metric_snapshots/{id}/approve`
- `POST /v1/commercial/13014/commercial_metric_snapshots/{id}/suspend`
- `POST /v1/commercial/13014/commercial_metric_snapshots/{id}/retire`
...
Required events: `metric.snapshot_created`, `forecast.updated`, `capacity.constraint_detected`, `metric.approved`, and `metric.superseded`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
