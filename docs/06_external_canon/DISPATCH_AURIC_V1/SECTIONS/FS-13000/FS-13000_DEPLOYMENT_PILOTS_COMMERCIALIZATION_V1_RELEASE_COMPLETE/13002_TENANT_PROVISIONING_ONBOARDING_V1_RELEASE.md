# 13002 — Tenant Provisioning & Onboarding V1

## Purpose

Define how a new institution, partner, vendor, or internal workspace is provisioned and activated.
...
- regulator/auditor guest environment
- internal Auric workspace
- synthetic demo tenant

## Provisioning Workflow

1. commercial/legal approval
2. tenant creation
3. region and environment assignment
4. domain verification
5. administrator invitation
6. identity configuration
7. role configuration
8. data-classification setup
9. connector selection
10. cartridge enablement
11. institution profile seeding
12. pilot workflow configuration
13. security review
14. user acceptance
15. activation
...
- Tenant creation is reproducible.
- Users cannot access before activation.
- Data and connector scopes are explicit.
- Roles and approvals are configured before pilot launch.
- Offboarding and suspension paths exist.
---
...
This volume is the binding V1 specification for **Tenant Provisioning & Onboarding V1**. It governs internal Auric operators, customers, implementation partners, automated services, and agents. Local statements of work and cartridge configurations may narrow scope or strengthen controls but may not bypass tenant isolation, evidence requirements, human decision authority, pricing approval, security review, or immutable audit history.

## Canonical Object and Field Dictionary

The primary object is `tenant_provisioning`. Every record MUST include `id`, `tenant_id`, `object_type`, `version`, `status`, `classification`, `owner_id`, `created_at`, `updated_at`, `effective_at`, `expires_at`, `source_refs`, `evidence_refs`, `approval_refs`, `control_refs`, `audit_ref`, `correlation_id`, and `idempotency_key`. Required domain fields are defined in `tenant_provisioning.schema.json`. Monetary fields MUST identify currency and basis; percentages MUST identify denominator and period; dates MUST use ISO 8601; unknown, stale, inferred, attested, synthetic, and verified values MUST remain distinguishable.
...
Canonical states: **requested, approved, provisioning, configured, uat, active, suspended, offboarding, closed**.

1. Creation begins in `requested` and confers no execution or commercial authority.
...
Minimum evidence includes: executed contract, tenant configuration, identity proof, data scope approval, activation checklist. Evidence MUST identify source, date, owner, classification, freshness, confidence, transformation history, reviewer, approval state, and integrity reference. Material decisions require a recorded rationale, alternatives considered, exceptions, compensating controls, and expiration or review date.

## API Contract

- `POST /v1/commercial/13002/tenant_provisionings`
- `GET /v1/commercial/13002/tenant_provisionings/{id}`
- `PATCH /v1/commercial/13002/tenant_provisionings/{id}`
- `POST /v1/commercial/13002/tenant_provisionings/{id}/submit`
- `POST /v1/commercial/13002/tenant_provisionings/{id}/approve`
- `POST /v1/commercial/13002/tenant_provisionings/{id}/suspend`
- `POST /v1/commercial/13002/tenant_provisionings/{id}/retire`
...
Required events: `tenant.requested`, `tenant.provisioned`, `tenant.configured`, `tenant.activated`, and `tenant.offboarded`. Event envelopes include `event_id`, `event_type`, `tenant_id`, `object_id`, `object_version`, `actor`, `occurred_at`, `correlation_id`, `classification`, `evidence_refs`, and `payload_hash`.
