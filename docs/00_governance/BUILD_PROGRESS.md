...
| Kernel services (**identity/permissions**, event bus, object registry live-persistence + governed contract, cost ledger, observability, request envelope + contracts/API, connector runtime + SDK, connector→registry candidate bridge, **canon reconciliation seam / alias registry**) | 20% | 62% | 12.4 |
| Truth/graph engines (resolver, confidence, live assembly, query, matured entity resolution, profile persistence, connector→observation bridge, at-scale persisted profiles, third_party_claim intake path, normalized connector output → Object Registry, **external-canon → repo identity reconciliation (propose-only, closed-graph)**) | 15% | 57% | 8.55 |
...
| Tests / observability / productionization | 5% | 65% | 3.25 |
| **Total** | **100%** | — | **~59%** |
