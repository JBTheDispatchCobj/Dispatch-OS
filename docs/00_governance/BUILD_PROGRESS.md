...
| Kernel services (**identity/permissions**, event bus, object registry live-persistence + governed contract, cost ledger, observability, request envelope + contracts/API, connector runtime + SDK, connector→registry candidate bridge, canon reconciliation seam / alias registry, UI surface registry (config-as-data product IA, closed-graph) **driving the registry-driven command palette + universal search (Terminal runtime, Vol VII)**) | 20% | 64% | 12.8 |
...
| Cooperative Markets cartridge (first product; /network surface orchestration; /institutions directory over the full market; **+ /opportunities over the deal engine + /workflows over the live work-item queue**) | 8% | 89% | 7.12 |
| Terminal (customer-facing product; whole FS-10000 IA framed as 23 surfaces + registry-driven nav; **THE TERMINAL RUNTIME — a registry-driven command palette (⌘K) + universal search — plus 16 live / 7 scaffold: /search, /opportunities, /workflows now real over live data**) | 7% | 72% | 5.04 |
| Tests / observability / productionization (**SEARCH + OPPORTUNITIES + WORKFLOWS debug steps; 348 unit tests**) | 5% | 71% | 3.55 |
| **Total** | **100%** | — | **~62%** |
