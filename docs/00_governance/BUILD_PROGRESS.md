...
| Kernel services (**identity/permissions**, event bus, object registry live-persistence + governed contract, cost ledger, observability, request envelope + contracts/API, connector runtime + SDK, connector→registry candidate bridge, canon reconciliation seam / alias registry, UI surface registry (config-as-data product IA, closed-graph) **driving the registry-driven command palette + universal search + the notification/task center read-model (Terminal runtime, Vol VII)**) | 20% | 64% | 12.8 |
| Truth/graph engines (resolver, confidence, live assembly, query, matured entity resolution, profile persistence, connector→observation bridge, at-scale persisted profiles, third_party_claim intake path, normalized connector output → Object Registry, external-canon → repo identity reconciliation (propose-only, closed-graph; **crosswalk 5→15**)) | 15% | 58% | 8.7 |
| Agent Harness + Execution Engine (router, planner, request-envelope run spine, runtime, evaluation, deal engine on live normalized intake P1/P2 **surfaced at /opportunities (ICApproval gate never auto-advances)**, connector→registry→resolve orchestration) | 15% | 29% | 4.35 |
| Publication (Auric) engine | 8% | 36% | 2.88 |
| Connector implementations + ingestion (generic runtime + SDK, config-as-data catalog 39→57→73→**93**, 6 real connectors live (NCUA 5300 + regs, SEC EDGAR, FDIC BankFind, Federal Register), full-market 5300 at scale, startup-intake, connector candidates → Object Registry) | 7% | 70% | 4.9 |
| Cooperative Markets cartridge (first product; /network surface orchestration; /institutions directory over the full market; /opportunities over the deal engine + /workflows over the live work-item queue; **+ live report objects + the report-sharing editorial gate seeded for /reports**) | 8% | 90% | 7.2 |
| Terminal (customer-facing product; whole FS-10000 IA framed as 23 surfaces + registry-driven nav; THE TERMINAL RUNTIME — command palette (⌘K) + universal search; **+ the notification/task center (Vol VII) on the Home command center + 18 live / 5 scaffold: /reports (report objects + editorial gate) + /cartridges (installed-config manifest) now real over live data**) | 7% | 78% | 5.46 |
| Tests / observability / productionization (**+ ACTIVITY + REPORTS + CARTRIDGES debug steps (25/25) with negative-control teeth; 363 unit tests**) | 5% | 73% | 3.65 |
| **Total** | **100%** | — | **~63%** |
...
## Changelog
- 2026-07-22 — **OLYMPIC SPRINT IV — WAVE 2: The Terminal RUNTIME (command palette + universal search, Vol VII) + promote 3 more scaffolds → REAL surfaces.**
  Additive; **no vertical noun in `core/`** (new pure builders in `app/_surfaces/`, the runner in `cartridges/`; only `ui_surfaces.json` config-as-data + `app/layout.tsx` + `app/globals.css` changed); connector runtime + engines + the human-gate contract layer UNCHANGED; default in-memory; look/feel deferred. New:
  (1) **`app/_surfaces/universal_search.ts`** — a pure, deterministic universal-search index + total-order matcher over three live collections (the /institutions directory rows, the UI surface registry, the canon aliases) + a registry-driven palette derivation. Doctrine states distinct: institution=**synthetic** (figures never presentable as real) · live surface=**current** · scaffold surface / proposed canon alias=**restricted** (framed/unconfirmed, not authoritative) · no-match=**missing** (honest empty state, never a fabricated hit). Erasable-only, import-type only.
  (2) **`components/terminal/TerminalShell.tsx`** — THE TERMINAL RUNTIME shell: a global **command palette** (⌘K / Ctrl-K) mounted once in `app/layout.tsx`, driven entirely from `ui_surfaces.json` (every surface a jump target; a new surface appears with NO code change), keyboard-navigable, hands a free-text query to `/search`. Pure navigation — never mutates/decides.
  (3) **`/search`** (scaffold→live) — `app/search/page.tsx` builds the index server-side from the live collections; `SearchView.tsx` ranks with `searchUniverse`, reads `?q=` client-side (page stays statically prerenderable). 
  (4) **`/opportunities`** (scaffold→live) — `app/_surfaces/opportunities_view.ts` (pure) + `cartridges/cooperative_markets/run_opportunities.ts` (runs the UNCHANGED intake→deal-engine path) render sourced, scored opportunities. Every score is a **Dispatch inference** (tier `dispatch_inference`), never a fact; the engine only RECOMMENDS — advancing to allocation requires the **ICApproval human gate** (proven by the pipeline), so a recommended-advance opportunity is **pending_approval**, NEVER auto-advanced to **current**; a blocked deal is **conflicted**. A triage surface with NO auto-advance control.
  (5) **`/workflows`** (scaffold→live) — `app/_surfaces/workflows_view.ts` (pure) groups the LIVE `WorkItem` objects by workflow `kind`, joins each to its cartridge WORKFLOW DEFINITION (config-as-data; label/owner/approvalRequired), rolls up status, and cross-references the live `Approval` objects by `related_work_item_id`. States distinct: **pending_approval** (a gate owed) · **conflicted** (blocked/rejected) · **current**; an unmapped kind is flagged (never mis-attached); NEVER decides a gate.
  (6) `ui_surfaces.json` flipped `/search`,`/opportunities`,`/workflows` scaffold→live (13→**16 live / 7 scaffold**; `/search` states gained `synthetic`); `app/globals.css` gained a command-palette overlay; new **SEARCH + OPPORTUNITIES + WORKFLOWS** debug-loop steps (renders-over-real-data · states distinct · human-gate-never-auto-decide · deterministic) + **16 unit tests** → **348**. Gate: debug-loop **ALL GREEN (22/22)** + `tsc` clean + `npm run build` exit 0 (26/26 routes prerender; the 3 promoted surfaces STATIC) + 348/348. **Adversarially self-reviewed** (4-lens: correctness · truth-doctrine/no-core-vertical-leak · purity/determinism/strip-safety · test-teeth) — 0 blockers (verified: no clock/random, no enums/parameter-properties, no `@/core/data` import in any gate-imported module, no `core/*.ts` vertical leak). Layers: **Terminal 57→72, kernel 63→64, harness 28→29, cartridge 88→89, tests 69→71**; headline **~61% → ~62%** (HONEST: a Terminal-runtime + surface-breadth wave; the ~80% Sprint-IV target still needs the notification/task centers, dashboard-runtime generalization, the remaining scaffolds, and harness/Auric depth). REAL bulk 5300 NOT staged → full market stays LABELED synthetic. No regulated conclusion in weights; human gates real (never auto-advance/auto-decide/hidden control).
- 2026-07-22 — **OLYMPIC SPRINT IV — WAVE 1: Promote 3 scaffolds → REAL surfaces over live data (Terminal & product surface begins).**
  Additive, no vertical noun in `core/` (new pure builders live in `app/_surfaces/`); the connector runtime + engines +
  the human-gate contract layer are UNCHANGED; default stays in-memory; look/feel deferred (reused the existing design
  tokens). Three of the highest-value FRAMED scaffolds became real surfaces over live run output, `ui_surfaces.json` the
  contract (flipped `/institutions`,`/approvals`,`/evidence` scaffold→live → **13 live / 10 scaffold**). New:
  (1) **`/institutions`** — `cartridges/cooperative_markets/run_institutions_directory.ts` (pure/deterministic/
  erasable-only) itemizes the whole synthetic market into browse rows (five 5300 ratios as deterministic calcs each
  citing its filing; asset-size + PCA readiness bands; profile confidence marked **inferred**; **region = `missing`** —
  not sourced from a 5300, shown never faked) + the shared pure `queryDirectory` (filter + total-order sort);
  `components/terminal/InstitutionsDirectoryView.tsx` renders search / filter / sort, each row → `/terminal`; scales
  one→thousands; figures LABELED synthetic (`all_labeled` computed from data; real bulk 5300 Bryan-only).
  (2) **`/approvals`** — `app/_surfaces/approvals_view.ts` buckets the store's `Approval` objects awaiting-vs-decided
  with lineage (governed work item/decision/proposal) + doctrine states (pending_approval / **restricted** = regulated,
  owner/admin-only / current); `components/terminal/ApprovalsView.tsx` renders the queue; approve/request-changes/reject
  post to the EXISTING `decideApprovalAction` → `app/contracts.decideApproval` → `core/kernel/permissions`
  (authorize-FIRST). The builder NEVER decides — only a `requested` approval is `decidable`.
  (3) **`/evidence`** — `app/_surfaces/evidence_view.ts` projects evidence across work items with lineage drill-through
  + states current / **stale** (aged `created_at`→`as_of`) / **inferred** (agent-captured) / pending_approval /
  **restricted** (doc-backed); `components/terminal/EvidenceView.tsx` groups by object / by source / unreviewed; review
  routes THROUGH `reviewEvidenceAction` → `contracts.reviewEvidence`. NEVER auto-reviews.
  (4) Additive coop-seed fixtures (1 pending high-risk approval + 1 unreviewed/old/agent-captured evidence) so the live
  gates render their FULL state legend over real store data (no test/debug step pins seed counts → regression-safe).
  (5) `app/actions.ts` gained `revalidatePath('/approvals')` + `('/evidence')`; new **INSTITUTIONS + APPROVALS +
  EVIDENCE** debug-loop steps (renders-over-real-data · human-gate-intact/never-auto-decide · missing/stale/inferred/
  synthetic/pending states distinct · deterministic); **+24 unit tests** → **332**. Gate: debug-loop **ALL GREEN
  (19/19)** + `tsc` clean + `npm run build` exit 0 (26/26 routes prerender; the 3 promoted surfaces STATIC) + 332/332.
  **Adversarially verified** (4-lens fleet: correctness · truth-doctrine/no-core-vertical-leak · purity/determinism ·
  test-teeth). Design note (DEBUG_LOG): the in-memory store uses TS parameter properties → not strip-safe, so the
  view-model builders are pure/store-free/erasable-only (tests + debug drive them with fixture arrays; the server pages
  read the store and hand arrays to the builder; Next compiles the store fully so pages render). Layers: **Terminal
  50→57, cartridge 87→88, tests 67→69**; headline **~60% → ~61%** (HONEST: a Terminal-surface breadth wave opening
  Sprint IV — the ~80% target needs the Terminal runtime (Vol VII) + more promoted scaffolds + harness/Auric depth
  across further waves). REAL bulk 5300 NOT staged → full market stays LABELED synthetic. No regulated conclusion in
  weights; truth tiers/planes not conflated; human gates real (never auto-approve/auto-review).
