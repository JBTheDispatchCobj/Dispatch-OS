...
| Kernel services (**identity/permissions**, event bus, object registry live-persistence + governed contract, cost ledger, observability, request envelope + contracts/API, connector runtime + SDK, connector→registry candidate bridge, canon reconciliation seam / alias registry, **UI surface registry (config-as-data product IA, closed-graph)**) | 20% | 63% | 12.6 |
...
| Cooperative Markets cartridge (first product; /network surface orchestration; **+ /institutions directory over the full market**) | 8% | 88% | 7.04 |
| Terminal (customer-facing product; whole FS-10000 IA framed as 23 surfaces + registry-driven nav; **13 live / 10 scaffold — /institutions directory + the LIVE /approvals + /evidence human-gate surfaces now real over live data**) | 7% | 57% | 3.99 |
| Tests / observability / productionization (**INSTITUTIONS + APPROVALS + EVIDENCE debug steps; 332 unit tests**) | 5% | 69% | 3.45 |
| **Total** | **100%** | — | **~61%** |
