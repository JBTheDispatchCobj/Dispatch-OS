...
| Kernel services (**identity/permissions**, event bus, object registry live-persistence + governed contract, cost ledger, observability, request envelope + contracts/API, **connector runtime + SDK**) | 20% | 60% | 12.0 |
| Truth/graph engines (resolver, confidence, live assembly, query, matured entity resolution, profile persistence, **connector→observation bridge**) | 15% | 47% | 7.05 |
...
| Connector implementations + ingestion (**generic runtime + SDK, config-as-data catalog, 2 real connectors live**) | 7% | 45% | 3.15 |
| Cooperative Markets cartridge (first product) | 8% | 80% | 6.4 |
| Terminal (customer-facing product) | 7% | 40% | 2.8 |
| Tests / observability / productionization | 5% | 57% | 2.85 |
| **Total** | **100%** | — | **~54%** |
...
2. **Full-market NCUA ingestion at scale** (Wave 2) — qualify the remaining catalog placeholders
   toward the ~93; wire a bulk 5300 acquire so the 5300 connector runs the WHOLE market (retiring
   `batch_fixtures.ts`) → institution profiles at scale persisted via the profile seam. Real bulk
   data is Bryan-only; the runtime + the persist/reconcile path are already proven. Truth 47→60,
   connectors 45→~65.
3. **Startup-intake connector** (Wave 2/3) feeding the deal engine (P1/P2) on real intake through
   the runtime — the DEFERRED "live intake path". Harness/cartridge up.
